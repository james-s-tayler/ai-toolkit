...
8. **Verify: run it with the run script, then verify with `prove-it-with-playwright`.** Launch the app via the run script (see *How to run*) — it installs, migrates, builds, and starts it; never build or launch it any other way. Then use the **`prove-it-with-playwright` skill** to confirm functionality still works after the merge: it drives the browser through login (Bearer token from `AI_TOOLKIT_AUTH`, stored in `localStorage`) and the key pages, asserting real data renders. (The UI run doesn't exercise training-side Python, so also `python -m py_compile` any touched `.py`.)
